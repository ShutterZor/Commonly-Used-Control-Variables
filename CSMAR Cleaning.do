log using "CSMAR CtrlVars Cleaning Log", replace name(ShutterZorADXMU20241231)
* =============================================================
/* Author Information */
	...
	/* 包含变量：BALANCE TOP1 */
		...
		*- 股东股本相关数据 - 1
		...
		drop in 1/2
			...
			*- 生成变量 - 第一大股东持股数量 TOP1
			...
			bys STKCD YEAR: egen TOP1 = max(S0304a)
		...
		*- 股东股本相关数据 - 3
		...
		drop in 1/2
			...
			*- 排序与保存数据
			order STKCD YEAR BALANCE TOP1
			label data "股东股本相关数据-3 - From Shutter Zor"
			save "DataB-Hub/Data7-3.dta", replace

		*- 合并股东股本相关数据
		use "DataB-Hub/Data7-1.dta", clear
		append using "DataB-Hub/Data7-2.dta"
		append using "DataB-Hub/Data7-3.dta"
